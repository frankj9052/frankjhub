...
export const TopContent = () => {
  ...
  const debouncedSearchChange = useDebouncedCallback((value?: string) => {
    if (value) {
      dispatch(actionSlice.actions.setSearchValue(value));
      dispatch(actionSlice.actions.cleanOffset());
    } else {
      dispatch(actionSlice.actions.cleanSearchValue());
    }
  }, 500);
  ...
  const handleStatusSelectionChange = (selection: SharedSelection) => {
    ...
    dispatch(actionSlice.actions.setStatusFilter(selectionArray as string[]));
  };
  const handleColumnSelectionChange = (keys: SharedSelection) => {
    const selectionArray = Array.from(keys);
    dispatch(actionSlice.actions.setVisibleColumn(selectionArray));
  };
  const handleCreateNewButtonClick = () => {
    setOpenModal(true);
  };
  const handlePageSizeChange = (e: ChangeEvent<HTMLSelectElement>) => {
    dispatch(actionSlice.actions.setLimit(Number(e.target.value)));
    dispatch(actionSlice.actions.cleanOffset());
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <FrankModal
        ...
        backdrop="opaque"
        body={<CreateActionForm onClose={() => setOpenModal(false)} />}
