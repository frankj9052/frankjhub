import { getResourceListAsync, useDispatch, useSelector } from '@/libs/redux';
import { createResource } from '@/services/resource.service';
import { ResourceCreateRequest, resourceCreateRequestSchema } from '@frankjhub/shared-schema';
...
export const CreateResourceForm = ({ onClose }: Props) => {
  ...
  } = useForm<ResourceCreateRequest>({
    resolver: zodResolver(resourceCreateRequestSchema),
    mode: 'onTouched',
  });

  const onSubmit = async (value: ResourceCreateRequest) => {
    const result = await createResource(value);
